...
class FileMenu:
    ...
    def on_menu_open(self):
        ...
        self.menu.add_command(label="新規作成 (⌘N)", command=self.new_file)
        self.menu.add_command(label="開く (⌘O)", command=self.open_file)
        self.menu.add_command(label="フォルダを開く", command=self.open_dir)
        ...
        self.menu.add_command(label="保存 (⌘S)", command=self.save_file)
        self.menu.add_command(label="名前を付けて保存", command=self.save_as_file)
        self.menu.add_command(label="すべて保存", command=self.save_all_file)

        self.menu.add_separator()

        self.menu.add_command(label="閉じる (⌘W)", command=self.close_file)
