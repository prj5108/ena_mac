...
class GenMenu:
    def __init__(self, form, ctx):
        ...
        self.form.win.bind("<Command-k>", lambda e: self._enable())
        #self.form.win.bind("<F4>", lambda e: self._disable())
        self.form.win.bind("<Command-l>", lambda e: self._disable())
        #self.form.win.bind("<Shift-F5>", lambda e: self._set_enabled(not self.ctx.generator.enabled))
        self.form.win.bind("<Command-j>", lambda e: self._set_enabled(not self.ctx.generator.enabled))
    ...
    def on_menu_open(self):
        ...
        self.menu.add_command(label="生成を開始 (⌘K)", command=self._enable)

        self.menu.add_command(label="生成を終了 (⌘L)", command=self._disable)
        ...
        self.menu.add_checkbutton(label="生成の開始/終了 (⌘J)", variable=self.enabled_var)

        self.menu.add_command(label="生成を中断", command=self.ctx.generator.abort)
